...
export const hasMacro: Macro<[TestStorageFactory]> = async (t, { factory }) => {
...
export const hasExpiredMacro: Macro<[TestStorageFactory]> = async (
  t,
  { factory }
) => {
  const storage = await factory(t, MIXED_SEED);
...
export const hasManyMacro: Macro<[TestStorageFactory]> = async (
  t,
  { factory }
) => {
  const storage = await factory(t, MIXED_SEED);
...
export const hasManyEmptyMacro: Macro<[TestStorageFactory]> = async (
  t,
  { factory }
) => {
  const storage = await factory(t, MIXED_SEED);
