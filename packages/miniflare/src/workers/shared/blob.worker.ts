...
function assertFullRange(range: InclusiveRange, contentLength: number) {
  assert(
    range.start === 0 && range.end === contentLength - 1,
    "Expected partial content for partial range, but received full content"
  );
}
...
async function fetchSingleRange(
  ...
  if (range !== undefined && res.status !== 206) {
    ...
    assertFullRange(range, contentLength);
  }
  return res.body;
}
...
async function writeMultipleRanges(
  ...
  for (let i = 0; i < ranges.length; i++) {
    ...
    // If we specified a range, but received full content, make sure the range
    // covered the full content
    if (res.status !== 206) assertFullRange(range, contentLength);
